            ...
            <div class="relative" x-data="{ dropdownOpen: false }" @click.outside="dropdownOpen = false">
                <a class="flex items-center text-gray-700 dark:text-gray-400" href="#"
                    ...
                    <span class="mr-3 h-11 w-11 overflow-hidden rounded-full">
                        <img src="{{ asset('storage/') }}/user/{{ auth()->user()->foto }}" alt="User" />
                    </span>
                    ...
                    <ul class="flex flex-col gap-1 border-b border-gray-200 pt-4 pb-3 dark:border-gray-800">
                        ...
                        <li x-data="{ openModal: false }">
                            <button @click="openModal = true"
                                class="group w-full text-theme-sm flex items-center gap-3 rounded-lg px-3 py-2 font-medium text-gray-700 hover:bg-gray-100 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-gray-300">
                                <svg class="fill-gray-500 group-hover:fill-gray-700 dark:fill-gray-400 dark:group-hover:fill-gray-300"
                                    width="24" height="24" viewBox="0 0 24 24" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                            ...
                            <div x-show="openModal" x-cloak
                                class="fixed inset-0 z-50 dark:text-white/90 dark:bg-dark-900 flex items-center justify-center bg-black bg-opacity-50">
                                <div @click.away="openModal = false"
                                    class="relative dark:text-white/90 dark:bg-gray-900 bg-white rounded-lg shadow-lg max-w-lg w-full p-6">
                                    <h3 class="text-lg font-semibold text-gray-900 mb-2 dark:text-white/90">Shift</h3>
                                    <div class="mb-5">

                                        <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
                                            <select x-ref="shiftSelect"
                                                @change="$refs.selectedShift.value = $event.target.value"
                                                name="role"
                                                ...
                                                @foreach ($shifts as $shift)
                                                    <option
                                                        {{ session('shift.id_shift') === $shift['id_shift'] ? 'selected' : '' }}
                                    ...
                                    <form method="POST" action="{{ route('set.shift') }}">
                                        @csrf
                                        <input type="hidden" name="id_shift" x-ref="selectedShift">
